...
import { importItem } from 'src/@core/utils/api/item/import'
import { validateItemFile } from 'src/@core/utils/api/item/validate-file'
...
const ImportItemModal = ({ isOpen, handleCloseModal }: Props) => {
  ...
  const importFile = useMutation({
    mutationFn: async (formData: any) => await importItem(formData),
    onSuccess: (data: any) => {
      queryClient.invalidateQueries([QUERY_INVOICE_KEYS.ITEMS_INVOICE])
  ...
  const validateFile = useMutation({
    mutationFn: async (formData: any) => await validateItemFile(formData),
  ...
  return (
    <Dialog open={isOpen} onClose={handleCloseModal} TransitionComponent={Fade} fullWidth maxWidth='md'>
      <DialogTitle textAlign='center'>Import Item</DialogTitle>
      <DialogContent>
        ...
        <Box>
          <ItemTable tableBody={bodyTable} tableHeader={headerTable} />
        </Box>
